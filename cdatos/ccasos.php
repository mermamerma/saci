	...
	class ccasos extends Datos 
{
    ...
    function insertar_caso($formulario)
    {
		...
		else
		{
			...
			if(validar_vacio($insertado)==false)
			{
				...
				$desc_seg=" <b>Registró</b> el Caso por primera Vez <b>(Id: {$this->get_idcaso()})</b>";
    ...
    function update_caso($formulario)
    {
		...
		# Seguimiento
		$this->set_idusuario_seg($_SESSION["idusuario"]);
        $desc_seg=" <b>Modificó</b> el Caso <b>(Id: {$this->get_idcaso()})</b>";
        $this->set_descripcion_seg($desc_seg);
        $this->set_idcaso_seg($this->get_idcaso());
        $this->crearSeguimiento();	
		#echo $sentencia_sql;
	...
	function getPuntosCuentas($idptocta)
    {
        ...
        if ($idptocta>0)        {
            $sql = "SELECT
			pc.*,
			to_char(pc.fecha_creacion,'dd/mm/YYYY') as fechacreacion,m.estatus,m.descripcion,
			u.firma, u.nombres, u.apellidos, f.* FROM
			puntos_cuenta pc
			LEFT JOIN sis_usuarios u ON pc.id_usuario = u.idusuario
			LEFT JOIN maestro m ON pc.id_decision = m.idmaestro ,
			funcionarios f WHERE id_caso = $idptocta ";			
			$rs_cuentas=$this->getRecordset($sql, $this->conn);
            return $rs_cuentas;
        }
        else
        {
