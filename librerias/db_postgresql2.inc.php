...
function Conectar(){
	 ...
	 //$B_strhost="172.16.100.27";
	 $B_strhost="172.25.104.222";
...
function Cargarlista($cadsql, $seleccion)
{
    ...
    while($Fields=$rs->Mostrar())
    {
        if($seleccion==$Fields[$rs->fn[1]])
            @$resultado=$resultado."<option value=".$Fields[$rs->fn[1]]." selected>".strtoupper($Fields[$rs->fn[2]])."</option>";
        else
            @$resultado=@$resultado."<option value=".$Fields[$rs->fn[1]].">".strtoupper($Fields[$rs->fn[2]])."</option>";
