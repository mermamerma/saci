...
function Conectar(){
	 ...
	 $B_strhost="172.16.100.29";
	 $B_strusuario="tierra_fertil";
	 $B_strpassword="123456";
...
function Cargarlista($cadsql, $seleccion){
    ...
    while($Fields=$rs->Mostrar())
    {
        if($seleccion==$Fields[$rs->fn[1]])
            $resultado=$resultado."<option value=".$Fields[$rs->fn[1]]." selected>".$Fields[$rs->fn[2]]."</option>";
        else
			...
			foreach ($Tabla as $obj=>$fila)
			{
				...
				else
				{
					...
					else
					{
                                            ...
                                            $resultado.="<td  align='".$fila["alineacion"]."'><a href=\"javascript:respuesta('".$registro[$fila["columna"]]."');\"><img  src=\"../comunes/imagenes/".$imagen."\" border=\"0\" title=\"".$fila["titulo_mensaje"]."\"/></a></td>";
	...
	for($nf=$primer_resultado;$nf<=$primer_resultado+$resultados_pagina-1;$nf++)
	{
		...
		if ($registro=$rstabla->Mostrar())
		{
			...
			foreach ($Tabla as $obj=>$fila)
			{
                            ...
                            $nanalista=$this->Count("casos_analistas","estatus_asignacion='0' and idcaso=".$registro[$campo_link]);
...
function Count($tabla,$condicion)
{
	...
	$rs=new Recordset($sql, $this->conn);
	$rs->Mostrar();
        if ($rs->rowset["indice"]>=0)   $nregistros=$rs->rowset["indice"];
        else $nregistros=0;
        return ($nregistros);
